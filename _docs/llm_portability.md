...
- xAI (Grok)
- Google Gemini
- OpenRouter (proxy to many models via one API)
...
llm/
  ...
  openai_compat.py  # OpenAICompatibleProvider (OpenAI, Grok, OpenRouter)
...
```
LLM_PROVIDER=anthropic        # anthropic | openai | grok | gemini | openrouter
...
- Only the API key for the **selected** provider needs to be set; others can
  be blank.
- **OpenRouter naming note**: OpenRouter's model strings are prefixed by
  origin provider, e.g. `anthropic/claude-3.5-sonnet` or `openai/gpt-4o` —
  worth a comment in `.env.example` so it's not confused with native
  Anthropic/OpenAI model strings.
...
this (OpenAI/Grok/OpenRouter support a JSON-mode flag; Gemini has its own;
...
3. Add `llm/openai_compat.py`, parameterized by base URL — wire up OpenAI
   first (most testable), then confirm Grok and OpenRouter work by only
...
- Do Grok and OpenRouter need any request-shape tweaks beyond base URL/model
